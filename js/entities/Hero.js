...
export class Hero extends Entity {
    constructor(data) {
        ...
        this.targetY = this.y;
        this.moving = false;
        this.movementSpeed = GridConfig.CELL_SIZE * 4; // 4 cells per second
    }
    ...
    moveToCell(gridX, gridY) {
        const newX = gridX * GridConfig.CELL_SIZE;
        const newY = gridY * GridConfig.CELL_SIZE;

        // Check if the target cell is within grid bounds
        if (newX >= 0 && newX < GridConfig.GRID_WIDTH * GridConfig.CELL_SIZE &&
            newY >= 0 && newY < GridConfig.GRID_HEIGHT * GridConfig.CELL_SIZE) {
            this.targetX = newX;
            this.targetY = newY;
            this.moving = true;
        }
